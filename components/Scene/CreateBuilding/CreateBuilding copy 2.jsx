...
const CreateBuilding = ({ cellHover }) => {
    ...
    const minX=width%2===0? (width)/2: (width+1)/2
    const minZ=height%2===0? (height)/2: (height+1)/2
    


    const pivotX = cellHover.z<minX? (minX+.5 )*CELL_SIZE:(cellHover.z+.5)*CELL_SIZE
    const pivotZ =( cellHover.x+.5) * CELL_SIZE
    
    const placeCursor=placeMarker(size)
    if (isAdding)
        return (
            <>
                <group position={[pivotX , .5, pivotZ]}>
                    {placeCursor}
                </group>
                <gridHelper
