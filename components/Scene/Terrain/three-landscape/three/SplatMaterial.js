...
export default class SplatStandardMaterialImpl extends MeshStandardMaterial {
    ...
    constructor(parameters) {
        ...
        this._normalWeights.value =
            this._normalWeights.value.length > 0
                ? this._normalWeights.value
                : new Array(12).fill("0.75")
        // todo estimate scale
    }
    ...
    onBeforeCompile(shader) {
        ...
        const { normalMaps, normalMap, diffuseMaps, splats, noise } =
            this.parameters

        if (!splats) {
            throw new Error(
                "splats is a required properties of SplatStandardMaterial"
            )
        }
...
const computeDiffuse = ({
    ...
    return diffuseMaps
        ...
        .map((diffuse, i) => {
            ...
            let alphaValue = `texture2D(splats[${splatIndex(i)}], vUv).${splatChannel(
                i
            )}`

            // optional modifiers
            if (saturation && i < saturation.length)
                colorValue = `czm_saturation(${colorValue}, saturation[${i}])`
            if (brightness && i < brightness.length)
                colorValue = `(${colorValue} + vec4(brightness[${i}], brightness[${i}], brightness[${i}], 0.0))`
...
const sampler2d = (name, data) =>
    data && data.length ? `uniform sampler2D ${name}[${data.length}];` : ""
const float = (name, data) =>
    data && data.length ? `uniform float ${name}[10];` : ""
